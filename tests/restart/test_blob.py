...
class BlobTestCase(NodeProvider, unittest.TestCase):
    ...
    def test_blob_record(self):
        ...
        with connect(node.http_url, error_trace=True) as conn:
            ...
            # After removal of the node.max_local_storage_nodes in 5.0, every node has it's own path.data generated by new_node
            filepath = Path(node._settings['path.data']).glob(f'**/{digest}')
